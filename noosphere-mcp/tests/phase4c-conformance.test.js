...
export const PHASE4C_SHARD = Object.freeze([
  ...
  'tests/restore-recovery-cli-crash.test.js',
  'tests/restore-recovery-cli-convergence.test.js',
  'tests/restore-recovery-cli-hostile-review.test.js',
  'tests/restore-recovery-cli-lock-policy.test.js',
...
const CONFORMANCE = Object.freeze({
  ...
  'crash recovery': {
    ...
    evidence: [
      ...
      ['tests/restore-recovery.test.js', 'waits for owner lock removal and converges'],
      ['tests/restore-recovery.test.js', 'requires owner intervention when post-rename destination bytes changed'],
      ['tests/restore-recovery.test.js', 'does not consume a journal-less candidate while its live apply holds the slot lock'],
      ['tests/restore-recovery.test.js', 'repeats the manifest barrier under the recovery lock'],
      ['tests/restore-recovery.test.js', 'never infers a destination replacement from a prepared journal'],
      ['tests/restore-recovery.test.js', 'leaves an abandoned lock byte-identical until the owner clears it'],
  ...
  'crash recovery reachability': {
    ...
    evidence: [
      ...
      ['tests/restore-recovery-cli-convergence.test.js', 'converges a SIGKILL at ${boundary} before a new apply after owner lock removal'],
      ['tests/restore-recovery-cli-convergence.test.js', 'never repeats a destination replacement across repeated CLI recovery'],
      ['tests/restore-recovery-cli-hostile-review.test.js', 'leaves a destination changed after the committed replacement untouched'],
      ['tests/restore-recovery-cli-hostile-review.test.js', 'converges a rename that committed before its journal event'],
      ['tests/restore-recovery-cli-hostile-review.test.js', 'requires owner intervention when the destination is neither pre-state nor replacement'],
      ['tests/restore-recovery-cli-hostile-review.test.js', 'releases a candidate stranded mid-apply with no journal'],
      ['tests/restore-recovery-cli-hostile-review.test.js', 'cannot strand a candidate that does not own its spent confirmation'],
      ['tests/restore-recovery-cli-hostile-review.test.js', 'requires owner intervention when the manifest moved after confirmation'],
    ],
  },
  ...
  'recovery lock policy': {
    implementation: ['continuity/internal/restore/recovery.js', 'recoverRestoreTransactions'],
    evidence: [
      ['tests/restore-recovery-cli-lock-policy.test.js', 'classifies liveness by process state alone, never by age or clock'],
      ['tests/restore-recovery-cli-lock-policy.test.js', 'depends on no host call that can refuse'],
      ['tests/restore-recovery-cli-lock-policy.test.js', 'exposes no clock or host seam the caller can steer'],
      ['tests/restore-recovery-cli-lock-policy.test.js', 'leaves a competitor lock untouched after the owner clears a stale lock'],
      ['tests/restore-recovery-cli-lock-policy.test.js', 'refuses a lock held by a live process without modifying it'],
      ['tests/restore-recovery-cli-lock-policy.test.js', 'fails closed on a malformed, unauthenticated, or foreign lock'],
      ['tests/restore-recovery-cli-lock-policy.test.js', 'does not touch a lock belonging to a different transaction'],
    ],
  },
  'package boundary': {
    implementation: ['continuity/trust-store.js', 'isSlotAuthoritative'],
