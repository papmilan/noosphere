...
// SEC-05 Phase 1 — authenticated, out-of-tree trust store.
//
...
// authoritative data.
//
// Phase 1 implements the store, machine key, canonical serialization, MAC,
// project-instance identity, record schema, and the fail-closed authority gate.
// It does NOT implement owner approval, restore, replay/freshness, or the
// normalizer — those are later phases. With no approval writer wired into any
// user flow, no production slot record exists, so every slot renders quoted
// (fail-closed) until a later phase adds an authenticated approval path.
...
export async function ensureMachineKey(env = process.env, secureFileOptions = {}) {
  const existing = await loadMachineKey(env, secureFileOptions).catch(() => null);
  if (existing) return existing;
  await ensureRealDirectoryPath(homeDir(env));
  const key = crypto.randomBytes(32);
  await atomicOwnerOnlyWrite(machineKeyPath(env), `${key.toString('hex')}\n`, secureFileOptions);
  return key;
}
...
async function readRecord(file, secureFileOptions) {
...
// exercised end-to-end. It is not wired into any user-facing flow in Phase 1.
...
// and is therefore completely untrusted.
