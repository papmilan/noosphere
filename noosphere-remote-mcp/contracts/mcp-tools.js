...
const warning = closed({ schema_version: { const: 'noosphere.project-memory/1.0.0' }, code: { enum: ['interrupted-session', 'checkpoint-predates-session'] }, message: text(320) });
const summary = closed({ schema_version: { const: 'noosphere.project-memory/1.0.0' }, current_status: text(), checkpoint_count: { type: 'integer', minimum: 0 }, session_count: { type: 'integer', minimum: 0 }, latest_checkpoint_id: nullable(id) });
